...
(function ($, ui) {
    ...
    ui.define('imguploader', {
        ...
        _init: function () {
            ...
            !$.isFunction(opts.filter) && (opts.filter = me._filterFn);

            return me;
        },
        _filterFn: function (files) {
            var fileArr = [], i, file;
        ...
        _imgEventHandler: function (e) {
            ...
            switch (e.type) {
                case 'change':
                    me._$waiting = $('<div class="mui-imguploader-waiting">读取中...</div>').css('opacity', '1').appendTo(me._tabs._getPanel());
        ...
        _renderThumbnail: function (e, i, len, file) {
            var me = this,
                ...
                $thumbnails = me._$thumbnails,
                tabs = me._tabs;

            $thumbnails.append(html[e.type]);
            if (i == len - 1) {
                me._$waiting.css('opacity', 0);
                tabs._getPanel().append($thumbnails).append(me._$uploadBtn.prop('disabled', false).removeClass('mui-state-disable'));
                tabs.refresh();
            }
        },
