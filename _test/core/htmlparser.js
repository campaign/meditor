...
test( '普通标签处理', function() {
    var root = ME.htmlparser('<i>sdfsdfsdfsf</i>');
    equals(root.toHtml(),'<i>sdfsdfsdfsf</i>','单个普通标签');
    root = ME.htmlparser('<i>sdf<b>sdfsdsd</b>fsdfsf</i>');
    equals(root.toHtml(),'<i>sdf<b>sdfsdsd</b>fsdfsf</i>','多个普通标签');
    root = ME.htmlparser('<i dsf="sdf" sdf="wewe" readonly >sdf</i>');
    equals(root.toHtml(),'<i dsf="sdf" sdf="wewe" readonly >sdf</i>','添加属性的标签');
    root = ME.htmlparser('<img src="file:///C:/DOCUME~1/DONGYA~1/LOCALS~1/Temp/msohtmlclip1/01/clip_image002.jpg" width="553" height="275" />');
...
test( '裸字', function() {
    var root = ME.htmlparser('sdfasdfasdf');
    equals(root.toHtml().replace(/[ ]+>/g,'>'),'sdfasdfasdf');

});
test( '只有结束标签的情况', function() {
    var root = ME.htmlparser('<p>hello1</a></p><p>hello2</p>');
