...
test( '合并;的问题', function () {
    equal(ME.utils.optCss( 'font-size:12px;&quot;;&lt;dssdfs&gt;;;' ),'font-size:12px;&quot;;&lt;dssdfs&gt;;','');
} ) ;
...
test( 'each 遍历方法', function () {
   ...
   ME.utils.each(div.getElementsByTagName('span'),function(node,i){
    ...
    equal(count,3);
    ME.utils.each(['a','b'],function(v,i){
        equal(v,['a','b'][i],'遍历数组');
    });
    ME.utils.each({a:1,b:2},function(v,k){
        equal(v,{a:1,b:2}[k],'遍历对象');
    });
} ) ;
test( 'clone 转换', function () {
    var obj = {a:1};
    var obj1= ME.utils.clone({a:1});
    obj.a = 2;
    equal(obj1.a,1);
    obj = {
        a:{
...
test( 'transUnitToPx 转换', function () {
    equal(ME.utils.transUnitToPx('20pt'),'27px');
    equal(ME.utils.transUnitToPx('0pt'),'0');
} ) ;
